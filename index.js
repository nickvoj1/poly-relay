...
async function getAuthedClient() {
  ...
  if (process.env.POLYMARKET_API_KEY && process.env.POLYMARKET_API_SECRET && process.env.POLYMARKET_PASSPHRASE) {
    ...
    cachedClient = new ClobClient(
      "https://clob.polymarket.com", 137, wallet, cachedCreds, sigType, funder,
    );
...
// POST /trade — sign + submit order using clob-client
      ...
      try {
        ...
        if (result.success) {
          ...
          return res.json({
            ...
            data: result, finalPrice, tickSize, attempt,
          });
        } else {
